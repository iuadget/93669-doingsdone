...
INSERT INTO tasks(user_id, project_id, created, deadline, name) VALUES (1, 1, CURDATE(), CURDATE()+ INTERVAL 3 DAY, 'Какая-то новая задача1');
/*Получить все задачи для завтрашнего дня*/
SELECT * FROM `tasks` WHERE (deadline = CURDATE() + INTERVAL 1 DAY) AND (dt_complete IS NOT 1);
